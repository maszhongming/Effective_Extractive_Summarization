...
def sequence_loss(logits, targets, sent_num, decoder, xent_fn=None, pad_idx=0):
    ...
    if decoder == 'PN':
        ...
        bs, seq_len, d = logits.size()
        target_len = targets.size(-1)
        assert d == 2

        sent_mask = len_mask(sent_num, logits.get_device())
        target = torch.LongTensor(bs, seq_len).to(logits.device)
        target.fill_(0)
        
        for i in range(bs):
            for j in range(target_len):
                if targets[i][j] != -1:
                    target[i][targets[i][j]] = 1
        sent_mask = sent_mask.contiguous().view(-1)
        target = target.contiguous().view(-1)
        logit = logits.contiguous().view(-1, logits.size(-1))
        
        target = target.masked_select(sent_mask).contiguous().view(-1)
        logit = logit.masked_select(sent_mask.unsqueeze(-1).expand_as(logit)).contiguous().view(-1, logit.size(-1))
        
        assert logit.size(-1) == 2
        assert logit.size()[:-1] == target.size()
